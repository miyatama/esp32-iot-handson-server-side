...
PROGNAME=$(basename $0)
VERSION="1.0"
IMAGE_NAME=stream-notificator:1.0
CONTAINER_NAME=stream-notificator
build_command=1
run_command=1
log_command=1
stop_command=1
console_command=1
notificate_endpoint="https://us-central1-esp32-iot-notificator-1f4cc.cloudfunctions.net/notificate"
...
function execute_run() {
  ...
  docker run \
    -it \
    -d \
    -e NOTIFICATE_ENDPOINT="${notificate_endpoint}" \
    --name ${CONTAINER_NAME} \
    ${IMAGE_NAME} ${@}
}
